...
<div class="container">
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="card-box">
                <form action="{{ route('airlienList.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="userName">NAME<span class="text-danger">*</span></label>
                        <input type="text" name="name" placeholder="ENTER AIRLINE NAME" class="form-control input-sm" value="{{ old('name') }}">
                    ...
                    <div class="form-group">
                        ...
                        <select name="membership_plan" class="form-control input-sm">
                            ...
                            <option value="YES" @if(old('membership_plan') == 'YES') selected @endif>YES</option>
                            <option value="NO" @if(old('membership_plan') == 'NO') selected @endif>NO</option>
                        </select>
                        ...
                        <select name="airline_group" class="form-control input-sm">
                            ...
                            @if (!(empty($airlineGroups)))
                                @foreach ($airlineGroups as $airlineGroup)
                                    <option value="{{ $airlineGroup['id'] }}" @if(old('airline_group') == $airlineGroup['id']) selected @endif>{{ $airlineGroup['name'] }}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="airline_gst" class="form-control input-sm" value="{{old('airline_gst')}}">
                            <option value="">SELECT AIRLINE GST</option>
                            <option value="0" @if(old('airline_gst') == '0') selected @endif>FROM AIRLINE</option>
                            <option value="1" @if(old('airline_gst') == '1') selected @endif>FROM WEBSITE</option>
                        </select>
...
@push('scripts')
    ...
    <script>
        $(document ).ready(function() {
            ...
            $('select[name="airline_gst"]').val('{!! old("airline_gst") !!}').trigger('change');
        });
    </script>
@endpush