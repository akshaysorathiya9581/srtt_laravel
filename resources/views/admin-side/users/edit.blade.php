            ...
            <div class="card-box">
                <form action="{{ route('users.update',$user->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="name" placeholder="ENTER USERNAME" class="form-control input-sm" value="{{ $user->name }}">
                        @if ($errors->has('name')) <p style="color:red;">{{ $errors->first('name') }}</p> @endif
                    </div>
                    <div class="form-group">
                        <label for="userName">EMAIL<span class="text-danger">*</span></label>
                        <input type="text" name="email" placeholder="ENTER EMAIL" class="form-control input-sm" value="{{ $user->email }}">
                        ...
                        <select name="roles" class="form-control input-sm">
                            ...
                            @if (!(empty($roles)))
                                @foreach ($roles as $role)
                                    <option value="{{ $role['id'] }}" @if(old('roles') == $user->role) selected @endif>{{ $role['name'] }}</option>
                                @endforeach
                            @endif
                        </select>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <label class="checkbox-inline">
                                    <input type="checkbox" name="permission[]" value="{{ $permission['id'] }}" {{ $checked }}> {{ $permission['name'] }} 
                                </label>
                            </div>
                        </div>
                    @endforeach
...
@push('scripts')
    ...
    <script>
        ...
        $('select[name="roles"]').val('{!! $user["roles"] !!}').trigger('change');
    </script>
@endpush
